...
<div class="comment">
<div class="label radius ctx-type">declaration</div>
<h1 id="Utils">Utils</h1><h5 class="subheader">Utils</h5>

<div class="description"></div>
<div class="tags">
    <ul class="no-bullet"></ul>
</div><pre><code class="language-javascript">var Utils = require("../../utils")
, util = require("util")
, tables = {}
, primaryKeys = {};
...
var query = 'SELECT column_name as "Field", column_default as "Default", is_nullable as "Null", data_type as "Type" FROM
...
options = options || {}
options.table = Array.isArray(tableName) ? tableName.map(function(t){return addQuotes(t);}).join(", ") :
addQuotes(tableName)
...
var query = "DELETE FROM <%= table %> WHERE <%= primaryKeys %> IN (SELECT <%= primaryKeysSelection %> FROM <%= table %>
...
var query = "SELECT relname FROM pg_class WHERE oid IN ( SELECT indexrelid FROM pg_index, pg_class WHERE
pg_class.relname='<%= tableName %>' AND pg_class.oid=pg_index.indrelid);"
...
var template = '<%= protocol %>://<%= user %>:<%= password %>@<%= host %><% if(port) { %>:<%= port %><% } %>/<%=
database %>';
