...
module.exports = (function () {
        ...
        var customEventEmitter = new Utils.CustomEventEmitter(function () {
            ...
            var primaryKeys = Utils._.keys(self.__factory.connectorDAO.rawAttributes)
                , foreignKey = primaryKeys.filter(function (pk) {
                    return pk != self.__factory.identifier
                })[0]
            ...
            self.__factory.target.findAllJoin(self.__factory.connectorDAO.tableName, options)
                .on('success', function (objects) {
                    customEventEmitter.emit('success', objects)
                })
                .on('error', function (err) {
                    customEventEmitter.emit('error', err)
                })
                .on('sql', function (sql) {
                    customEventEmitter.emit('sql', sql)
                })
        })
    ...
    HasManyDoubleLinked.prototype.injectSetter = function (emitterProxy, oldAssociations, newAssociations) {
        ...
        destroyObsoleteAssociations
            ...
            .on('sql', function (sql) {
                emitterProxy.emit('sql', sql)
            })
            .error(function (err) {
                emitterProxy.emit('error', err)
            })
            .success(function () {
                var chainer = new Utils.QueryChainer
                    ...
                    , unassociatedObjects = newAssociations.filter(function (obj) {
                        return !obj.equalsOneOf(oldAssociations)
                    })
    ...
    var destroyObsoleteAssociations = function (oldAssociations, newAssociations) {
        ...
        return new Utils.CustomEventEmitter(function (emitter) {
            ...
            var obsoleteAssociations = oldAssociations.filter(function (obj) {
                return !obj.equalsOneOf(newAssociations)
            })
